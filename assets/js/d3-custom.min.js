// Custom d3 bundle for the Atlas — concatenated standalone UMD builds, not a
// hand-edited/re-bundled d3.js. Each module below is d3's own unmodified
// dist/d3-*.min.js from unpkg, in dependency order (each only needs modules
// already loaded above it — see the shared-global wiring note in each UMD
// header). Regenerate by re-fetching these exact versions if atlas-map.js
// starts using a new d3.* call; do not hand-edit the contents below.
//
// Modules (all pinned to what d3@7.9.0's package.json range resolves to):
//   d3-array        3.2.4  (d3.mean; d3-geo's own dependency)
//   d3-color        3.1.0  (d3-interpolate's dependency)
//   d3-dispatch     3.0.1  (d3-transition/d3-drag/d3-zoom's dependency)
//   d3-ease         3.0.1  (d3-transition's dependency)
//   d3-timer        3.0.1  (d3-transition's dependency)
//   d3-interpolate  3.0.1  (d3-transition/d3-zoom's dependency)
//   d3-selection    3.0.0  (d3.select; d3-transition/d3-drag/d3-zoom's dependency)
//   d3-transition   3.0.1  (.transition().duration(); d3-zoom's dependency)
//   d3-drag         3.0.0  (d3-zoom's dependency, for its own pointer handling)
//   d3-zoom         3.0.0  (d3.zoom, d3.zoomIdentity)
//   d3-geo          3.1.1  (d3.geoMercator, d3.geoPath)
//   d3-fetch        3.0.1  (d3.json — its d3-dsv dependency is unused/omitted,
//                          since only .json() is called, not .csv()/.tsv())
